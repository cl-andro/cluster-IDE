...
Here is the base script to build clusterIDE:
...
- Linux: https://github.com/cl-andro/cluster-IDE/blob/master/.github/workflows/stable-linux.yml
- macOS: https://github.com/cl-andro/cluster-IDE/blob/master/.github/workflows/stable-macos.yml
- Windows: https://github.com/cl-andro/cluster-IDE/blob/master/.github/workflows/stable-windows.yml
...
# verify the snap
review-tools.snap-review --allow-classic cluster*.snap
```
...
- when the script pauses at `Press any key when the conflict have been resolved...`, open `vscode` directory in **clusterIDE**
...
- open `vscode` directory in a new **clusterIDE**'s window
- fix all the `*.rej` files
- run `npm run watch`
- run `./script/code.sh` until everything is ok
