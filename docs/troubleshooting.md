...
- [Windows](#windows)
  ...
  - ["Open with clusterIDE" missing from context menu](#windows-context-menu)
...
rm -rf ~/snap/cluster/common/.cache
fc-cache -r
```

### <a id="linux-rendering-glitches"></a>_Text and/or the entire interface not appearing_

You have likely encountered [a bug in Chromium and Electron](microsoft/vscode#190437) when compiling Mesa shaders, which has affected all Visual Studio Code and clusterIDE versions for Linux distributions since 1.82. The current workaround (see microsoft/vscode#190437) is to delete the GPU cache as follows:

```bash
rm -rf ~/.config/clusterIDE/GPUCache
```
...
Credits: [Gerson](https://gitlab.com/paulcarroty/clusterIDE-deb-rpm-repo/-/issues/91)
...
Use the clusterIDE's compatible extension [Open Remote - SSH](https://open-vsx.org/extension/jeanp413/open-remote-ssh).
...
- try the command `cluster --verbose`
...
- try `cluster --ozone-platform=x11`

## <a id="windows"></a>Windows

### <a id="windows-gpo"></a>_Group Policy Objects (GPOs) are ignored_

clusterIDE uses its own policy-watcher library (`@clusterIDE/policy-watcher`) which reads GPO values from a **different registry path** than VS Code.

**clusterIDE reads policies from:**

```
HKLM\SOFTWARE\Policies\clusterIDE\clusterIDE
```
...
If you are deploying clusterIDE in an enterprise environment via Group Policy:
...
3. Open `gpedit.msc` and configure policies under the clusterIDE group
4. Verify the resulting registry key exists at `HKLM\SOFTWARE\Policies\clusterIDE\clusterIDE` (not `Microsoft\clusterIDE`)

If you set policies manually via Registry Editor, make sure you create the key at the correct path:

```
HKLM\SOFTWARE\Policies\clusterIDE\clusterIDE\<PolicyName>  (REG_SZ or REG_DWORD)
```

For example, to set `Update: Mode` to `none`:

```
Registry key: HKLM\SOFTWARE\Policies\clusterIDE\clusterIDE
Value name:   update.mode
Value type:   REG_SZ
Value data:   none
```

Per-user policies are also supported under `HKCU\SOFTWARE\Policies\clusterIDE\clusterIDE` (machine policies take precedence).

### <a id="windows-context-menu"></a>_"Open with clusterIDE" missing from context menu_

If the **"Open with clusterIDE"** option does not appear after installation (even with the checkbox checked during setup):

1. **Run the installer again** and ensure _"Add 'Open with clusterIDE' action to Windows Explorer file context menu"_ is checked.
2. **Windows 11 note**: Windows 11 hides most context menu entries behind **Shift + Right-click** ("Show more options"). clusterIDE's entry may be present but hidden in the new condensed menu. Try Shift + Right-click to see the classic context menu.
3. If the entry still does not appear, you can add it manually via Registry Editor:

   ```
   Key:   HKEY_CLASSES_ROOT\*\shell\Open with clusterIDE
   Value: (Default) = "Open with clusterIDE"

   Key:   HKEY_CLASSES_ROOT\*\shell\Open with clusterIDE\command
   Value: (Default) = "C:\Program Files\clusterIDE\clusterIDE.exe" "%1"
   ```
...
Some users report Windows Defender detecting the clusterIDE installer as `Cinjo` or another threat. This is a **false positive** caused by the unsigned nature of certain build artifacts.

- Download clusterIDE **only from the official [GitHub Releases page](https://github.com/cl-andro/cluster-IDE/releases)**.
