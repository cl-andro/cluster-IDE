...
export APP_NAME="Cluster"
export APP_NAME_LC="cluster"
export ASSETS_REPOSITORY="ClusterFamily/cluster"
export BINARY_NAME="cluster"
export CI_BUILD="no"
export GH_REPO_PATH="ClusterFamily/cluster"
export ORG_NAME="ClusterFamily"
...
while getopts ":ilops" opt; do
  case "$opt" in
    i)
      export ASSETS_REPOSITORY="ClusterFamily/cluster-insiders"
      export BINARY_NAME="cluster-insiders"
...
if [[ "${SKIP_BUILD}" == "no" ]]; then
  if [[ "${SKIP_SOURCE}" != "no" ]]; then
    ...
    while [[ -n "$( git log -1 | grep "CLUSTER IDE HELPER" )" ]]; do
      git reset -q --hard HEAD~
    done
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    ...
    if [[ -f "${HOME}/.gyp/include.gypi" ]]; then
      mv ~/.gyp/include.gypi ~/.gyp/include.gypi.pre-cluster
    else
      echo "{}" > ~/.gyp/include.gypi.pre-cluster
    fi
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    mv ~/.gyp/include.gypi.pre-cluster ~/.gyp/include.gypi
  fi
