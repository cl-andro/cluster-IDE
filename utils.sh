...
APP_NAME="${APP_NAME:-Cluster}"
APP_NAME_LC="${APP_NAME_LC:-$( echo "${APP_NAME}" | awk '{print tolower($0)}' | sed 's/ /-/g' )}"
ASSETS_REPOSITORY="${ASSETS_REPOSITORY:-ClusterFamily/cluster}"
BINARY_NAME="${BINARY_NAME:-cluster}"
GH_REPO_PATH="${GH_REPO_PATH:-ClusterFamily/cluster}"
ORG_NAME="${ORG_NAME:-ClusterFamily}"
